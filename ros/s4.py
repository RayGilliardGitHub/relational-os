...
class S4Service:
    def __init__(self, substrate: Substrate, label: str = "qk"):
        ...
        self.label = label       # org/path segment for URIs this service builds (§C1)
    ...
    def settle(self, exchange: dict, i: int, signer: str = "agent://s4") -> dict:
        ...
        asset = {
            "uri": f"asset://money/{self.label}-escrow-{slug}",
            "kind": "MONEY",
        ...
        obligation = {
            "uri": f"obligation://{self.label}/s4-pay-{slug}",
        ...
        receipt = {
            "uri": f"receipt://{self.label}/s4-receipt-{slug}",
        ...
        reconciliation = {
            "uri": f"decision://{self.label}/s4-recon-{slug}",
            "by": signer,
            "authority": f"authority://{self.label}/for-settlement",
            "alternatives": ["reject"],
            "confidence": 1.0,
        ...
        exch_event = self._ev(
            "EXCHANGE", f"event://{self.label}/s4-exchange-{slug}", signer, i,
    ...
    def evaluate(self, exchange: dict, expectation: dict, i: int,
        ...
        outcome = self._ev(
            "OUTCOME", f"event://{self.label}/s4-outcome-{slug}", signer, i,
