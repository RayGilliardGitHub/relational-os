...
# Each command must run from inside sprint-5/artifacts/ (runners are CWD-sensitive).
set -u
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="${1:-$SCRIPT_DIR/..}"
ARTS=$ROOT/sprints/sprint-5/artifacts
VENV=$ROOT/sprints/sprint-0/artifacts/.venv/bin/python
FLAGS=0

if [ ! -d "$ARTS" ]; then echo "ERR: no artifacts dir at $ARTS"; exit 2; fi

step() { echo; echo "== $1 =="; }

step "1) daily cockpit (S1-S5 + BOL, writes graph/ledger/cockpit)"
( cd "$ARTS" && python3 run_s5_demo.py > /tmp/relos-demo.txt 2>&1 )
ec=$?; echo "exit=$ec last=$(tail -1 /tmp/relos-demo.txt)"; [ $ec -ne 0 ] && FLAGS=$((FLAGS+1))

if [ -x "$VENV" ]; then
  step "2) conformance all-six generations"
  ( cd "$ARTS" && "$VENV" run_s5_conformance.py > /tmp/relos-conf.txt 2>&1 )
  ...
  echo "WARN: Sprint-0 venv missing at $VENV — rebuild per docs/02-setup (python3 -m venv + pip install jsonschema referencing pyyaml)"
  FLAGS=$((FLAGS+1))
fi
