...
# ============================================================================
# SPRINT 5 — Business Operating Layer checks
# ============================================================================
from .bol import BolService, project_on_time  # noqa: E402

CASE_URI = "case://qk/c-on-time-delivery"
TASK_R = "task://qk/t-provider-rebalance"
POLICY = "policy://qk/provider-allocation"
LEARN = "decision://qk/s5-learning-on-time"


def s5_bol_check(sub) -> list[tuple[str, bool, str]]:
    """5.1: the case completes its legal lifecycle with the exception heartbeat, becomes
    assigned authorized work, closes in a verified outcome, and records a Learning entry."""
    results: list[tuple[str, bool, str]] = []
    r = lambda name, ok, detail="": results.append((name, ok, detail))  # noqa: E731
    case = sub.graph.get(CASE_URI)
    r("case:// on graph", case is not None, repr(case.get("uri") if case else None))
    hist = [h["status"] for h in (case or {}).get("history", [])]
    expected_seq = ["OPEN", "TRIAGE", "ASSIGNED", "IN_PROGRESS", "BLOCKED",
                    "RESOLVED", "CLOSED"]
    r("case lifecycle completes OPEN->...->CLOSED (legal transitions)",
      hist == expected_seq, str(hist))
    r("exception heartbeat populated (§7J.2 EXPECTED->...->VERIFIED)",
      case is not None and all(k in case for k in
          ("expected", "actual", "variance", "significance", "exception",
           "root_cause", "recommended_action", "verified_outcome"))
      and case.get("significance") == "CRITICAL",
      f"significance={case.get('significance') if case else None}, "
      f"variance={case.get('variance') if case else None}")
    r("root-cause carries an epistemic status (§7K.2)",
      case is not None and case.get("root_cause_status") == "SUPPORTED",
      f"status={case.get('root_cause_status') if case else None}")
    task = sub.graph.get(TASK_R)
    r("#8 becomes assigned authorized Task (owner worker + authority)",
      task is not None and task.get("assigned_to") == "agent://w-ops"
      and task.get("authority") == "authority://qk/for-operations"
      and isinstance(task.get("priority"), (int, float)),
      f"assigned_to={task.get('assigned_to') if task else None}, "
      f"priority={task.get('priority') if task else None}")
    learn = sub.graph.get(LEARN)
    r("Learning entry (§7K.1 Decision->Expected->Actual->Variance->WHY->change-future-policy)",
      learn is not None and all(k in learn for k in
          ("expected", "actual", "why", "change_future_policy")),
      f"why={str(learn.get('why'))[:50] if learn else None}")
    ev = sub.graph.get("evidence://qk/job-routed-solarworks")
    r("verified outcome evidence captured (rallied delivery on time)",
      ev is not None and bool(ev.get("verity")), repr(ev.get("uri") if ev else None))
    pol = sub.graph.get(POLICY)
    r("future policy updated by Learning (version 3 + learning ref)",
      pol is not None and pol.get("version") == 3
      and pol.get("learning") == LEARN, f"version={pol.get('version') if pol else None}")
    return results


def business_health_check(sub) -> list[tuple[str, bool, str]]:
    """5.2: metrics carry ledger-derived actual/variance; priority is a deterministic
    ordered score; dependencies + impact analysis cover the exception->case->task chain."""
    results: list[tuple[str, bool, str]] = []
    r = lambda name, ok, detail="": results.append((name, ok, detail))  # noqa: E731
    m = sub.graph.get("metric://qk/m-on-time")
    mt = sub.graph.get("metric://qk/m-customer-trust")
    mv = sub.graph.get("metric://qk/m-settled-value")
    r("metrics carry actual + variance (ledger-derived)",
      m is not None and mt is not None and mv is not None
      and all(isinstance(x.get("actual"), (int, float))
              and isinstance(x.get("variance"), (int, float))
              for x in (m, mt, mv)),
      f"on-time actual={m.get('actual') if m else None}, "
      f"trust actual={mt.get('actual') if mt else None}, "
      f"value actual={mv.get('actual') if mv else None}")
    r("on-time metric actual matches the ledger projection",
      m is not None and abs(m["actual"] - round(project_on_time(sub)[0] / max(1, project_on_time(sub)[1]), 3)) < 1e-9,
      f"actual={m.get('actual') if m else None}")
    att = sub._meta.get("s52", {}).get("attention", [])
    scores = [a["priority"] for a in att]
    r("attention is priority-ordered (deterministic score)",
      scores == sorted(scores, reverse=True) and len(att) >= 2, str(scores))

    # priority is deterministic — same inputs -> same score
    bol = BolService(sub)
    p1 = bol.priority(0.8, 1.0, 0.5, False, 0.4, 0.6)
    p2 = bol.priority(0.8, 1.0, 0.5, False, 0.4, 0.6)
    r("Priority = f(impact,urgency,confidence,irreversibility,relationship,cost-of-delay) deterministic",
      p1["score"] == p2["score"] and 0 <= p1["score"] <= 1, f"score={p1['score']}")

    deps = [o for o in sub.graph.objects.values()
            if o.get("uri", "").startswith("dependency://")]
    kinds = {d.get("kind") for d in deps}
    r("Dependencies represented (requires/blocks/enables/derived_from/impacts)",
      {"REQUIRES", "BLOCKS", "ENABLES", "DERIVED_FROM", "IMPACTS"} <= kinds,
      f"kinds={sorted(kinds)}")
    imp = bol.impact_analysis(TASK_R)
    r("impact analysis traces the exception->case->task chain (what breaks if the task fails)",
      CASE_URI in imp["impacted"] and "metric://qk/m-on-time" in imp["impacted"]
      and "goal://qk/g-customer-trust" in imp["impacted"],
      f"impacted={imp['impacted']}")
    health = sub._meta.get("s52", {}).get("health", [])
    r("business-health panel derives from ledger-projected metrics",
      len(health) == 3 and all("status" in h for h in health),
      f"statuses={[h['status'] for h in health]}")
    return results


def cockpit_check(sub) -> list[tuple[str, bool, str]]:
    """5.3: the cockpit shows business health, prioritized attention, an AI
    recommendation with authority (#8 assigned work), and #10 verified outcome+Learning."""
    results: list[tuple[str, bool, str]] = []
    r = lambda name, ok, detail="": results.append((name, ok, detail))  # noqa: E731
    c = sub._meta.get("s53", {})
    r("cockpit health panel present", bool(c.get("health")) and len(c["health"]) == 3,
      f"{len(c.get('health', []))} metrics")
    r("cockpit shows prioritized attention (§7J.2 'things today')",
      bool(c.get("attention")) and c["attention"] == sorted(
          c["attention"], key=lambda a: -a["priority"]),
      f"top={c['attention'][0]['uri'] if c.get('attention') else None}")
    rec = c.get("recommendation", {})
    r("AI recommendation carries the authority it requires (§7J.9)",
      bool(rec.get("summary")) and rec.get("authority_required") == "authority://qk/for-operations"
      and rec.get("includes_do_nothing") is True, f"auth={rec.get('authority_required')}")
    r("#8 is assigned, authorized work in the ledger (task + decision)",
      sub.graph.get(TASK_R) is not None and sub.graph.get("decision://qk/s5-assign") is not None,
      "task://qk/t-provider-rebalance + decision://qk/s5-assign")
    vo = c.get("verified_outcome", {})
    lr = c.get("learning")
    r("#10 verified outcome + Learning recorded",
      vo.get("evaluation") == "met" and vo.get("forward_on_time") == 1.0 and bool(lr),
      f"forward_on_time={vo.get('forward_on_time')}, learning={lr}")
    r("§7L report written (cockpit.md + cockpit.json)",
      (Path(__file__).resolve().parent.parent / "reports/cockpit.md").exists()
      and (Path(__file__).resolve().parent.parent / "reports/cockpit.json").exists(),
      "reports/")
    return results


def _s5_checks():
    return {"bol": s5_bol_check, "health": business_health_check,
            "cockpit": cockpit_check}