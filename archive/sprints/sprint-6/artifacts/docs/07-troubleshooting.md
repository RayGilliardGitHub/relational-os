...
- **Fix:** use `/home/rlg/relational-os/sprints/sprint-0/artifacts/.venv/bin/python` for any
...
### F4. The demo (`run_s5_demo.py`) exits non-zero or prints `FAILURES PRESENT`
...
Conformance is the authoritative gate. Restore a clean workspace (or your working copy), then:
    ...
    /home/rlg/relational-os/sprints/sprint-0/artifacts/.venv/bin/python run_s5_conformance.py   # expect ALL PASS
...
issue (the released reference build passed all six generations: Sprint-0 156 / -1 28 / -2 35 /
-3 55 / -4 174 / -5 316 instances).

### F6. The cockpit report is stale or missing
- **Cause:** `reports/cockpit.md(+.json)` are **outputs** written by `run_s5_demo.py`.
- **Fix:** re-run `cd /home/rlg/relational-os/sprints/sprint-5/artifacts && python3 run_s5_demo.py`;
